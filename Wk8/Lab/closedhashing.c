...
ListNode* HashSearch(HashTable Q4Hash, int key)
{
    if(Q4Hash.hSize == 0)
        return NULL;
    int idx = Hash(key, Q4Hash.hSize);
    ListNode *temp = Q4Hash.Table[idx].head;
    while(temp != NULL)
    {
        if (temp->key == key)
            return temp;
...
int HashInsert(HashTable* Q4HashPtr, int key)
{
    if (HashSearch(*Q4HashPtr, key) != NULL)
        return 0;
    int idx = Hash(key, Q4HashPtr->hSize);
    ListNode *temp = Q4HashPtr->Table[idx].head;
    ListNode *new = (ListNode*) malloc(sizeof(ListNode));
    new->key = key;
    new->next = temp;
    Q4HashPtr->Table[idx].head = new;
    Q4HashPtr->Table[idx].size ++;
    Q4HashPtr->nSize ++;
    return 1;
}
// {
//  // Write your code here
//     // serach for node first, if exists, dont insert
//     ListNode *search = HashSearch(*Q4HashPtr, key);
//     if (search != NULL)
//         return 0;
